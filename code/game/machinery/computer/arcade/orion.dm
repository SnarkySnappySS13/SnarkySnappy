		...
		if("buycrew") //buy a crewmember
			if(!spaceport_raided && food >= 10 && fuel >= 10 && gameStatus == ORION_STATUS_MARKET)
				if(params["odd"])
					...
					add_crewmember(pick(GLOB.commando_names + GLOB.thougher_names + GLOB.ai_names + GLOB.clown_names + GLOB.mime_names + GLOB.plasmaman_names + GLOB.ethereal_names + GLOB.carp_names))
				else
					add_crewmember()
