...
/obj/structure/noose/deconstruct(disassembled = TRUE, wirecutters_disassembly = 0)
	new /obj/item/stack/cable_coil (drop_location(), 30)
	qdel(src)
...
/obj/structure/noose/post_buckle_mob(mob/living/carbon/buckled)
	...
	if(ishuman(buckled))
		...
		height_offset += buckled_human.mob_height - buckled_human.mob_height
	animate(buckled, pixel_y = initial(pixel_y) + height_offset, time = 8, easing = LINEAR_EASING)
	ADD_TRAIT(buckled, TRAIT_FORCED_STANDING, BUCKLED_TRAIT) //messes with the animation, also youre hanging how you gonna stagger
...
/obj/structure/noose/post_unbuckle_mob(mob/living/buckled)
	...
	REMOVE_TRAIT(buckled, TRAIT_FORCED_STANDING, BUCKLED_TRAIT)
...
/obj/structure/noose/user_buckle_mob(mob/living/carbon/human/victim, mob/user, check_loc = TRUE)
	...
	if(user != victim)
		to_chat(user, span_notice("It will take 10 seconds and you have to stand still."))
	if(do_after(user, user == victim ? 0 : 10 SECONDS, victim))
